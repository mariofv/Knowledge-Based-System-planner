
(defclass %3ACLIPS_TOP_LEVEL_SLOT_CLASS "Fake class to save top-level slot information"
	(is-a USER)
	(role abstract)
	(single-slot Country+name
;+		(comment "The name of a country.")
		(type STRING)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Days
;+		(comment "The number of days in which the visitor visits the museum.")
		(type INTEGER)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Observation+time
		(type INTEGER)
		(default -1)
;+		(cardinality 0 1)
		(create-accessor read-write))
	(multislot Preferences
;+		(comment "The preferences of a visitor")
		(type INSTANCE)
;+		(allowed-classes Author Period Style Topic)
		(create-accessor read-write))
	(single-slot Relevance
;+		(comment "The relevance of a painting.")
		(type INTEGER)
		(range 0 100)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Children
;+		(comment "Whether the visiting group has children or no.")
		(type SYMBOL)
		(allowed-values FALSE TRUE)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Painting+topic
;+		(comment "The topic of a painting.")
		(type INSTANCE)
;+		(allowed-classes Topic)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Period+name
;+		(comment "The name of the historical period.")
		(type STRING)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Visitor+name
		(type STRING)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Asigned+time
;+		(comment "The time asigned to a day.")
		(type INTEGER)
;+		(cardinality 0 1)
		(create-accessor read-write))
	(single-slot Painted+in
;+		(comment "The period in which a painting was painted.")
		(type INSTANCE)
;+		(allowed-classes Period)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Width
;+		(comment "The width of a painting.")
		(type INTEGER)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Created+by
;+		(comment "The author of the painting.")
		(type INSTANCE)
;+		(allowed-classes Author)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Number
;+		(comment "The number of a day.")
		(type INTEGER)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Visitor+interest
		(type INTEGER)
		(default -1)
;+		(cardinality 0 1)
		(create-accessor read-write))
	(single-slot Nationality
		(type INSTANCE)
;+		(allowed-classes Country)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Year+of+creation
;+		(comment "The year in wich the painting was created.")
		(type INTEGER)
;+		(cardinality 0 1)
		(create-accessor read-write))
	(single-slot Topic+name
		(type STRING)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Duration
;+		(comment "The duration in hours of the visits of a visitor.")
		(type INTEGER)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Number+of+people
;+		(comment "The number of people that composes a group.")
		(type INTEGER)
		(range 1 100)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Painting+name
;+		(comment "The name of a painting.")
		(type STRING)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(multislot Asigned+paintings
		(type INSTANCE)
;+		(allowed-classes Painting)
		(create-accessor read-write))
	(single-slot Style+name
;+		(comment "The name of an style.")
		(type STRING)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Complexity
;+		(comment "The complexity of a painting.")
		(type INTEGER)
;+		(cardinality 0 1)
		(create-accessor read-write))
	(single-slot Exhibited+in
;+		(comment "The room in which a painting was exhibited.")
		(type INSTANCE)
;+		(allowed-classes Room)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Knowledge
;+		(comment "The knowledge of a visitor")
		(type INTEGER)
		(range 0 100)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Painting+style
;+		(comment "The pictorial period in which the painting was painted.")
		(type INSTANCE)
;+		(allowed-classes Style)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Height
;+		(comment "The height of a painting.")
		(type INTEGER)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Author+name
;+		(comment "The name of an author.")
		(type STRING)
;+		(cardinality 1 1)
		(create-accessor read-write)))

(defclass Painting "A painting that is exposed in a museum."
	(is-a USER)
	(role concrete)
	(single-slot Observation+time
		(type INTEGER)
		(default -1)
;+		(cardinality 0 1)
		(create-accessor read-write))
	(single-slot Painting+style
;+		(comment "The pictorial period in which the painting was painted.")
		(type INSTANCE)
;+		(allowed-classes Style)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Relevance
;+		(comment "The relevance of a painting.")
		(type INTEGER)
		(range 0 100)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Painted+in
;+		(comment "The period in which a painting was painted.")
		(type INSTANCE)
;+		(allowed-classes Period)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Width
;+		(comment "The width of a painting.")
		(type INTEGER)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Height
;+		(comment "The height of a painting.")
		(type INTEGER)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Created+by
;+		(comment "The author of the painting.")
		(type INSTANCE)
;+		(allowed-classes Author)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Visitor+interest
		(type INTEGER)
		(default -1)
;+		(cardinality 0 1)
		(create-accessor read-write))
	(single-slot Painting+name
;+		(comment "The name of a painting.")
		(type STRING)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Year+of+creation
;+		(comment "The year in wich the painting was created.")
		(type INTEGER)
;+		(cardinality 0 1)
		(create-accessor read-write))
	(single-slot Complexity
;+		(comment "The complexity of a painting.")
		(type INTEGER)
;+		(cardinality 0 1)
		(create-accessor read-write))
	(single-slot Exhibited+in
;+		(comment "The room in which a painting was exhibited.")
		(type INSTANCE)
;+		(allowed-classes Room)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Painting+topic
;+		(comment "The topic of a painting.")
		(type INSTANCE)
;+		(allowed-classes Topic)
;+		(cardinality 1 1)
		(create-accessor read-write)))

(defclass Author "Person that creates art."
	(is-a USER)
	(role concrete)
	(single-slot Nationality
		(type INSTANCE)
;+		(allowed-classes Country)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Author+name
;+		(comment "The name of an author.")
		(type STRING)
;+		(cardinality 1 1)
		(create-accessor read-write)))

(defclass Period "A period in history."
	(is-a USER)
	(role concrete)
	(single-slot Period+name
;+		(comment "The name of the historical period.")
		(type STRING)
;+		(cardinality 1 1)
		(create-accessor read-write)))

(defclass Style "Style / School / Pictorial Period"
	(is-a USER)
	(role concrete)
	(single-slot Style+name
;+		(comment "The name of an style.")
		(type STRING)
;+		(cardinality 1 1)
		(create-accessor read-write)))

(defclass Room "A room of the museum."
	(is-a USER)
	(role concrete)
	(multislot Asigned+paintings
		(type INSTANCE)
;+		(allowed-classes Painting)
		(create-accessor read-write))
	(single-slot Number
;+		(comment "The number of a day.")
		(type INTEGER)
;+		(cardinality 1 1)
		(create-accessor read-write)))

(defclass Visitor "A visitor of a museum."
	(is-a USER)
	(role concrete)
	(single-slot Duration
;+		(comment "The duration in hours of the visits of a visitor.")
		(type INTEGER)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Days
;+		(comment "The number of days in which the visitor visits the museum.")
		(type INTEGER)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Number+of+people
;+		(comment "The number of people that composes a group.")
		(type INTEGER)
		(range 1 100)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Visitor+name
		(type STRING)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(multislot Preferences
;+		(comment "The preferences of a visitor")
		(type INSTANCE)
;+		(allowed-classes Author Period Style Topic)
		(create-accessor read-write))
	(single-slot Knowledge
;+		(comment "The knowledge of a visitor")
		(type INTEGER)
		(range 0 100)
;+		(cardinality 1 1)
		(create-accessor read-write))
	(single-slot Children
;+		(comment "Whether the visiting group has children or no.")
		(type SYMBOL)
		(allowed-values FALSE TRUE)
;+		(cardinality 1 1)
		(create-accessor read-write)))

(defclass Topic "A topic."
	(is-a USER)
	(role concrete)
	(single-slot Topic+name
		(type STRING)
;+		(cardinality 1 1)
		(create-accessor read-write)))

(defclass Day "A day of the visit."
	(is-a USER)
	(role concrete)
	(multislot Asigned+paintings
		(type INSTANCE)
;+		(allowed-classes Painting)
		(create-accessor read-write))
	(single-slot Asigned+time
;+		(comment "The time asigned to a day.")
		(type INTEGER)
;+		(cardinality 0 1)
		(create-accessor read-write))
	(single-slot Number
;+		(comment "The number of a day.")
		(type INTEGER)
;+		(cardinality 1 1)
		(create-accessor read-write)))

(defclass Country "A country of the world."
	(is-a USER)
	(role concrete)
	(single-slot Country+name
;+		(comment "The name of a country.")
		(type STRING)
;+		(cardinality 1 1)
		(create-accessor read-write)))

(definstances Instancias 

([Ontologia_Class0] of  Painting

	(Created+by [Ontologia_Class1])
	(Exhibited+in [Ontologia_Class40004])
	(Height 77)
	(Observation+time 20)
	(Painted+in [Ontologia_Class10000])
	(Painting+name "La Gioconda")
	(Painting+style [Ontologia_Class3])
	(Painting+topic [Ontologia_Class10008])
	(Relevance 100)
	(Visitor+interest -1)
	(Width 53)
	(Year+of+creation 1519))

([Ontologia_Class1] of  Author

	(Author+name "Leonardo da Vinci")
	(Nationality [Ontologia_Class50003]))

([Ontologia_Class10000] of  Period

	(Period+name "Edad Media"))

([Ontologia_Class10002] of  Room

	(Number 1))

([Ontologia_Class10008] of  Topic

	(Topic+name "Retrato"))

([Ontologia_Class20000] of  Painting

	(Created+by [Ontologia_Class20001])
	(Exhibited+in [Ontologia_Class40003])
	(Height 95)
	(Observation+time 8)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "El caminante sobre el mar de nubes")
	(Painting+style [Ontologia_Class20003])
	(Painting+topic [Ontologia_Class20002])
	(Relevance 75)
	(Visitor+interest -1)
	(Width 75)
	(Year+of+creation 1818))

([Ontologia_Class20001] of  Author

	(Author+name "Caspar David Friedrich")
	(Nationality [Ontologia_Class50004]))

([Ontologia_Class20002] of  Topic

	(Topic+name "Paisaje"))

([Ontologia_Class20003] of  Style

	(Style+name "Romanticismo"))

([Ontologia_Class20004] of  Period

	(Period+name "Edad Moderna"))

([Ontologia_Class3] of  Style

	(Style+name "Renacimiento"))

([Ontologia_Class30000] of  Room

	(Number 2))

([Ontologia_Class30002] of  Period

	(Period+name "Edad Contemporanea"))

([Ontologia_Class30003] of  Style

	(Style+name "PostImpresionismo"))

([Ontologia_Class30004] of  Style

	(Style+name "Expresionismo"))

([Ontologia_Class30005] of  Style

	(Style+name "Simbolismo"))

([Ontologia_Class30006] of  Style

	(Style+name "Surrealismo"))

([Ontologia_Class30007] of  Style

	(Style+name "Neoclasicismo"))

([Ontologia_Class30008] of  Topic

	(Topic+name "Amor"))

([Ontologia_Class30009] of  Topic

	(Topic+name "Religion"))

([Ontologia_Class30010] of  Topic

	(Topic+name "Muerte"))

([Ontologia_Class30011] of  Topic

	(Topic+name "Juramento"))

([Ontologia_Class30012] of  Author

	(Author+name "Vincent van Gogh")
	(Nationality [Ontologia_Class50014]))

([Ontologia_Class30013] of  Painting

	(Created+by [Ontologia_Class30012])
	(Exhibited+in [Ontologia_Class40003])
	(Height 92)
	(Observation+time 66)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "La noche estrellada")
	(Painting+style [Ontologia_Class30003])
	(Painting+topic [Ontologia_Class20002])
	(Relevance 100)
	(Visitor+interest -1)
	(Width 74)
	(Year+of+creation 1889))

([Ontologia_Class30014] of  Painting

	(Created+by [Ontologia_Class30015])
	(Exhibited+in [Ontologia_Class10002])
	(Height 91)
	(Observation+time 9)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "El Grito")
	(Painting+style [Ontologia_Class30004])
	(Painting+topic [Ontologia_Class10008])
	(Relevance 100)
	(Visitor+interest -1)
	(Width 74)
	(Year+of+creation 1893))

([Ontologia_Class30015] of  Author

	(Author+name "Edvard Munch")
	(Nationality [Ontologia_Class50006]))

([Ontologia_Class30017] of  Author

	(Author+name "Gustav Klimt")
	(Nationality [Ontologia_Class50008]))

([Ontologia_Class30018] of  Painting

	(Created+by [Ontologia_Class30017])
	(Exhibited+in [Ontologia_Class10002])
	(Height 180)
	(Observation+time 42)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "El beso")
	(Painting+style [Ontologia_Class30005])
	(Painting+topic [Ontologia_Class30008])
	(Relevance 70)
	(Visitor+interest -1)
	(Width 180)
	(Year+of+creation 1907))

([Ontologia_Class30019] of  Author

	(Author+name "Salvador Dali")
	(Nationality [Ontologia_Class50009]))

([Ontologia_Class30020] of  Painting

	(Created+by [Ontologia_Class30019])
	(Exhibited+in [Ontologia_Class30000])
	(Height 33)
	(Observation+time 69)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "La Persistencia de la Memoria")
	(Painting+style [Ontologia_Class30006])
	(Painting+topic [Ontologia_Class20002])
	(Relevance 90)
	(Visitor+interest -1)
	(Width 24)
	(Year+of+creation 1931))

([Ontologia_Class30021] of  Author

	(Author+name "Sandro Botticelli")
	(Nationality [Ontologia_Class50003]))

([Ontologia_Class30022] of  Painting

	(Created+by [Ontologia_Class30021])
	(Exhibited+in [Ontologia_Class40003])
	(Height 173)
	(Observation+time 90)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "El nacimiento de Venus")
	(Painting+style [Ontologia_Class3])
	(Painting+topic [Ontologia_Class30009])
	(Relevance 85)
	(Visitor+interest -1)
	(Width 279)
	(Year+of+creation 1484))

([Ontologia_Class30023] of  Author

	(Author+name "Antonello da Messina")
	(Nationality [Ontologia_Class50003]))

([Ontologia_Class30024] of  Painting

	(Created+by [Ontologia_Class30023])
	(Exhibited+in [Ontologia_Class10002])
	(Height 74)
	(Observation+time 5)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "Cristo muerto sostenido por un angel")
	(Painting+style [Ontologia_Class3])
	(Painting+topic [Ontologia_Class30009])
	(Relevance 20)
	(Visitor+interest -1)
	(Width 51)
	(Year+of+creation 1475))

([Ontologia_Class30025] of  Author

	(Author+name "Theodore Gericault")
	(Nationality [Ontologia_Class50010]))

([Ontologia_Class30026] of  Painting

	(Created+by [Ontologia_Class30025])
	(Exhibited+in [Ontologia_Class10002])
	(Height 491)
	(Observation+time 50)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "La Balsa de la Medusa")
	(Painting+style [Ontologia_Class20003])
	(Painting+topic [Ontologia_Class30010])
	(Relevance 75)
	(Visitor+interest -1)
	(Width 717)
	(Year+of+creation 1819))

([Ontologia_Class30027] of  Author

	(Author+name "Jacques-Louis David")
	(Nationality [Ontologia_Class50010]))

([Ontologia_Class30028] of  Painting

	(Created+by [Ontologia_Class30027])
	(Exhibited+in [Ontologia_Class10002])
	(Height 330)
	(Observation+time 24)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "El Juramento de los Horacios")
	(Painting+style [Ontologia_Class30007])
	(Painting+topic [Ontologia_Class30011])
	(Relevance 45)
	(Visitor+interest -1)
	(Width 425)
	(Year+of+creation 1784))

([Ontologia_Class40000] of  Painting

	(Created+by [Ontologia_Class30012])
	(Exhibited+in [Ontologia_Class30000])
	(Height 71)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Lirios de agua")
	(Painting+style [Ontologia_Class30003])
	(Painting+topic [Ontologia_Class40002])
	(Relevance 80)
	(Visitor+interest -1)
	(Width 93)
	(Year+of+creation 1889))

([Ontologia_Class40002] of  Topic

	(Topic+name "Flores"))

([Ontologia_Class40003] of  Room

	(Number 3))

([Ontologia_Class40004] of  Room

	(Number 4))

([Ontologia_Class40005] of  Painting

	(Created+by [Ontologia_Class40006])
	(Exhibited+in [Ontologia_Class40003])
	(Height 121)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "El portador de flores")
	(Painting+style [Ontologia_Class40007])
	(Painting+topic [Ontologia_Class40002])
	(Relevance 40)
	(Visitor+interest -1)
	(Width 121)
	(Year+of+creation 1935))

([Ontologia_Class40006] of  Author

	(Author+name "Diego Rivera")
	(Nationality [Ontologia_Class50005]))

([Ontologia_Class40007] of  Style

	(Style+name "Realismo"))

([Ontologia_Class40008] of  Painting

	(Created+by [Ontologia_Class40009])
	(Exhibited+in [Ontologia_Class40004])
	(Height 74)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Gotico estadounidense")
	(Painting+style [Ontologia_Class40010])
	(Painting+topic [Ontologia_Class10008])
	(Relevance 52)
	(Visitor+interest -1)
	(Width 62)
	(Year+of+creation 1930))

([Ontologia_Class40009] of  Author

	(Author+name "Grant Wood")
	(Nationality [Ontologia_Class50007]))

([Ontologia_Class40010] of  Style

	(Style+name "Regionalismo"))

([Ontologia_Class40011] of  Painting

	(Created+by [Ontologia_Class30012])
	(Exhibited+in [Ontologia_Class40004])
	(Height 81)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Terraza de cafe por la noche")
	(Painting+style [Ontologia_Class30003])
	(Painting+topic [Ontologia_Class20002])
	(Relevance 90)
	(Visitor+interest -1)
	(Width 65)
	(Year+of+creation 1888))

([Ontologia_Class40012] of  Painting

	(Created+by [Ontologia_Class40013])
	(Exhibited+in [Ontologia_Class30000])
	(Height 116)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "El hijo del hombre")
	(Painting+style [Ontologia_Class30006])
	(Painting+topic [Ontologia_Class10008])
	(Relevance 60)
	(Visitor+interest -1)
	(Width 89)
	(Year+of+creation 1964))

([Ontologia_Class40013] of  Author

	(Author+name "Rene Magritte")
	(Nationality [Ontologia_Class50013]))

([Ontologia_Class40014] of  Painting

	(Created+by [Ontologia_Class40015])
	(Exhibited+in [Ontologia_Class40003])
	(Height 240)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "No. 5, 1948")
	(Painting+style [Ontologia_Class30004])
	(Painting+topic [Ontologia_Class40017])
	(Relevance 50)
	(Visitor+interest -1)
	(Width 120)
	(Year+of+creation 1948))

([Ontologia_Class40015] of  Author

	(Author+name "Jackson Pollock")
	(Nationality [Ontologia_Class50007]))

([Ontologia_Class40016] of  Style

	(Style+name "Abstracto"))

([Ontologia_Class40017] of  Topic

	(Topic+name "Abstracto"))

([Ontologia_Class40018] of  Painting

	(Created+by [Ontologia_Class40020])
	(Exhibited+in [Ontologia_Class30000])
	(Height 131)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Baile en el Moulin de la Galette")
	(Painting+style [Ontologia_Class40019])
	(Painting+topic [Ontologia_Class20002])
	(Relevance 40)
	(Visitor+interest -1)
	(Width 174)
	(Year+of+creation 1876))

([Ontologia_Class40019] of  Style

	(Style+name "Impresionismo"))

([Ontologia_Class40020] of  Author

	(Author+name "Pierre-August Renoir")
	(Nationality [Ontologia_Class50010]))

([Ontologia_Class40021] of  Painting

	(Created+by [Ontologia_Class40022])
	(Exhibited+in [Ontologia_Class40004])
	(Height 80)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Blanco sobre Blanco")
	(Painting+style [Ontologia_Class40023])
	(Painting+topic [Ontologia_Class40017])
	(Relevance 0)
	(Visitor+interest -1)
	(Width 80)
	(Year+of+creation 1918))

([Ontologia_Class40022] of  Author

	(Author+name "Kasimir Malevich")
	(Nationality [Ontologia_Class50011]))

([Ontologia_Class40023] of  Style

	(Style+name "Supermatismo"))

([Ontologia_Class50000] of  Painting

	(Created+by [Ontologia_Class50001])
	(Exhibited+in [Ontologia_Class10002])
	(Height 583)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "A Beauty")
	(Painting+style [Ontologia_Class50002])
	(Painting+topic [Ontologia_Class10008])
	(Relevance 10)
	(Visitor+interest -1)
	(Width 800)
	(Year+of+creation 1536))

([Ontologia_Class50001] of  Author

	(Author+name "Tiziano")
	(Nationality [Ontologia_Class50003]))

([Ontologia_Class50002] of  Style

	(Style+name "Manierismo"))

([Ontologia_Class50003] of  Country

	(Country+name "Italy"))

([Ontologia_Class50004] of  Country

	(Country+name "German"))

([Ontologia_Class50005] of  Country

	(Country+name "Mexico"))

([Ontologia_Class50006] of  Country

	(Country+name "Norway"))

([Ontologia_Class50007] of  Country

	(Country+name "United States of America"))

([Ontologia_Class50008] of  Country

	(Country+name "Hungary"))

([Ontologia_Class50009] of  Country

	(Country+name "Spain"))

([Ontologia_Class50010] of  Country

	(Country+name "France"))

([Ontologia_Class50011] of  Country

	(Country+name "Russia"))

([Ontologia_Class50013] of  Country

	(Country+name "Belgium"))

([Ontologia_Class50014] of  Country

	(Country+name "Netherlands"))

([Ontologia_Class60003] of  Painting

	(Created+by [Ontologia_Class60004])
	(Exhibited+in [Ontologia_Class30000])
	(Height 65)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "A casa do pintor en Presqueiras")
	(Painting+style [Ontologia_Class40007])
	(Painting+topic [Ontologia_Class20002])
	(Relevance 5)
	(Visitor+interest -1)
	(Width 76)
	(Year+of+creation 1873))

([Ontologia_Class60004] of  Author

	(Author+name "Virxilio Blanco")
	(Nationality [Ontologia_Class50009]))

([Ontologia_Class60005] of  Painting
    
    (Created+by [Ontologia_Class40022])
	(Exhibited+in [Ontologia_Class40003])
	(Height 500)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "A Forest Pool")
	(Painting+style [Ontologia_Class50002])
	(Painting+topic [Ontologia_Class20002])
	(Relevance 15)
	(Visitor+interest -1)
	(Width 596)
	(Year+of+creation 1595))

([Ontologia_Class60007] of  Author

	(Author+name "Paul Brill")
	(Nationality [Ontologia_Class50013]))

([Ontologia_Class60008] of  Painting

	(Created+by [Ontologia_Class60009])
	(Exhibited+in [Ontologia_Class40004])
	(Height 546)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "Adoration of the Magi")
	(Painting+style [Ontologia_Class50002])
	(Painting+topic [Ontologia_Class30009])
	(Relevance 10)
	(Visitor+interest -1)
	(Width 800)
	(Year+of+creation 1623))

([Ontologia_Class60009] of  Author

	(Author+name "Abraham Bloemaert")
	(Nationality [Ontologia_Class60010]))

([Ontologia_Class60010] of  Country

	(Country+name "Holland"))

([Ontologia_Class60011] of  Painting

	(Created+by [Ontologia_Class60012])
	(Exhibited+in [Ontologia_Class10002])
	(Height 800)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "Aeneas' Flight from Troy")
	(Painting+style [Ontologia_Class50002])
	(Painting+topic [Ontologia_Class60013])
	(Relevance 20)
	(Visitor+interest -1)
	(Width 555)
	(Year+of+creation 1598))

([Ontologia_Class60012] of  Author

	(Author+name "Federico Barocci")
	(Nationality [Ontologia_Class50003]))

([Ontologia_Class60013] of  Topic

	(Topic+name "Mitologia"))

([Ontologia_Class60014] of  Painting

	(Created+by [Ontologia_Class60016])
	(Exhibited+in [Ontologia_Class30000])
	(Height 61)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "Albert Cuyper")
	(Painting+style [Ontologia_Class60015])
	(Painting+topic [Ontologia_Class10008])
	(Relevance 10)
	(Visitor+interest -1)
	(Width 45)
	(Year+of+creation 1632))

([Ontologia_Class60015] of  Style

	(Style+name "Holland School"))

([Ontologia_Class60016] of  Author

	(Author+name "Rembrandt Harmenszoon van Rijn")
	(Nationality [Ontologia_Class50014]))

([Ontologia_Class60017] of  Painting

	(Created+by [Ontologia_Class60018])
	(Exhibited+in [Ontologia_Class40003])
	(Height 102)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "Alegoría de la Fe")
	(Painting+style [Ontologia_Class50002])
	(Painting+topic [Ontologia_Class60013])
	(Relevance 8)
	(Visitor+interest -1)
	(Width 78)
	(Year+of+creation 1520))

([Ontologia_Class60018] of  Author

	(Author+name "Alessandro Bonvicino")
	(Nationality [Ontologia_Class50003]))

([Ontologia_Class60019] of  Painting

	(Created+by [Ontologia_Class60020])
	(Exhibited+in [Ontologia_Class40004])
	(Height 140)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Alegoria de la Poesia")
	(Painting+style [Ontologia_Class30005])
	(Painting+topic [Ontologia_Class60013])
	(Relevance 30)
	(Visitor+interest -1)
	(Width 184)
	(Year+of+creation 1895))

([Ontologia_Class60020] of  Author

	(Author+name "Santiago Rusinyol")
	(Nationality [Ontologia_Class50009]))

([Ontologia_Class60021] of  Painting

	(Created+by [Ontologia_Class30012])
	(Exhibited+in [Ontologia_Class10002])
	(Height 54)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Angel")
	(Painting+style [Ontologia_Class30003])
	(Painting+topic [Ontologia_Class60022])
	(Relevance 0)
	(Visitor+interest -1)
	(Width 64)
	(Year+of+creation 1889))

([Ontologia_Class60022] of  Topic

	(Topic+name "Costumbres"))

([Ontologia_Class60023] of  Painting

	(Created+by [Ontologia_Class60024])
	(Exhibited+in [Ontologia_Class30000])
	(Height 775)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "Annunciazione")
	(Painting+style [Ontologia_Class50002])
	(Painting+topic [Ontologia_Class30009])
	(Relevance 3)
	(Visitor+interest -1)
	(Width 800)
	(Year+of+creation 1545))

([Ontologia_Class60024] of  Author

	(Author+name "Domenico Beccafumi")
	(Nationality [Ontologia_Class50003]))

([Ontologia_Class60026] of  Painting

	(Created+by [Ontologia_Class30012])
	(Exhibited+in [Ontologia_Class40003])
	(Height 75)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Anochecer")
	(Painting+style [Ontologia_Class30003])
	(Painting+topic [Ontologia_Class60022])
	(Relevance 10)
	(Visitor+interest -1)
	(Width 94)
	(Year+of+creation 1889))

([Ontologia_Class60027] of  Painting

	(Created+by [Ontologia_Class60028])
	(Exhibited+in [Ontologia_Class40004])
	(Height 83)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Arlequin Pensativo")
	(Painting+style [Ontologia_Class30005])
	(Painting+topic [Ontologia_Class10008])
	(Relevance 40)
	(Visitor+interest -1)
	(Width 61)
	(Year+of+creation 1901))

([Ontologia_Class60028] of  Author

	(Author+name "Pablo Picasso")
	(Nationality [Ontologia_Class50009]))

([Ontologia_Class60029] of  Painting

	(Created+by [Ontologia_Class30012])
	(Exhibited+in [Ontologia_Class10002])
	(Height 32)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Autoretrato")
	(Painting+style [Ontologia_Class30003])
	(Painting+topic [Ontologia_Class10008])
	(Relevance 70)
	(Visitor+interest -1)
	(Width 23)
	(Year+of+creation 1887))

([Ontologia_Class60030] of  Painting

	(Created+by [Ontologia_Class60016])
	(Exhibited+in [Ontologia_Class30000])
	(Height 616)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "Autoretrato con baston")
	(Painting+style [Ontologia_Class60015])
	(Painting+topic [Ontologia_Class10008])
	(Relevance 20)
	(Visitor+interest -1)
	(Width 800)
	(Year+of+creation 1658))

([Ontologia_Class60031] of  Painting

	(Created+by [Ontologia_Class60032])
	(Exhibited+in [Ontologia_Class40003])
	(Height 66)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "Autoretrato")
	(Painting+style [Ontologia_Class60033])
	(Painting+topic [Ontologia_Class10008])
	(Relevance 20)
	(Visitor+interest -1)
	(Width 80)
	(Year+of+creation 1643))

([Ontologia_Class60032] of  Author

	(Author+name "Diego Velazquez")
	(Nationality [Ontologia_Class50009]))

([Ontologia_Class60033] of  Style

	(Style+name "Barroco"))

([Ontologia_Class60034] of  Painting

	(Created+by [Ontologia_Class30012])
	(Exhibited+in [Ontologia_Class40004])
	(Height 71)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Barcazas de carbon")
	(Painting+style [Ontologia_Class30003])
	(Painting+topic [Ontologia_Class60022])
	(Relevance 0)
	(Visitor+interest -1)
	(Width 95)
	(Year+of+creation 1888))

([Ontologia_Class60035] of  Painting

	(Created+by [Ontologia_Class60036])
	(Exhibited+in [Ontologia_Class10002])
	(Height 79)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Blue Water Lilies")
	(Painting+style [Ontologia_Class30003])
	(Painting+topic [Ontologia_Class20002])
	(Relevance 40)
	(Visitor+interest -1)
	(Width 80)
	(Year+of+creation 1916))

([Ontologia_Class60036] of  Author

	(Author+name "Claude Oscar Monet")
	(Nationality [Ontologia_Class50010]))

([Ontologia_Class60037] of  Painting

	(Created+by [Ontologia_Class60039])
	(Exhibited+in [Ontologia_Class30000])
	(Height 46)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "Bodegon")
	(Painting+style [Ontologia_Class60038])
	(Painting+topic [Ontologia_Class60022])
	(Relevance 40)
	(Visitor+interest -1)
	(Width 84)
	(Year+of+creation 1653))

([Ontologia_Class60038] of  Style

	(Style+name "Escuela espanyola"))

([Ontologia_Class60039] of  Author

	(Author+name "Francisco de Zurbaran")
	(Nationality [Ontologia_Class50009]))

([Ontologia_Class60040] of  Painting

	(Created+by [Ontologia_Class60041])
	(Exhibited+in [Ontologia_Class40003])
	(Height 19)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "Camino al mercado")
	(Painting+style [Ontologia_Class50002])
	(Painting+topic [Ontologia_Class20002])
	(Relevance 10)
	(Visitor+interest -1)
	(Width 26)
	(Year+of+creation 1603))

([Ontologia_Class60041] of  Author

	(Author+name "Jan Brueghel el Mozo")
	(Nationality [Ontologia_Class50010]))

([Ontologia_Class60042] of  Painting

	(Created+by [Ontologia_Class60043])
	(Exhibited+in [Ontologia_Class40004])
	(Height 800)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "Chicken Vendors")
	(Painting+style [Ontologia_Class50002])
	(Painting+topic [Ontologia_Class60022])
	(Relevance 0)
	(Visitor+interest -1)
	(Width 545)
	(Year+of+creation 1580))

([Ontologia_Class60043] of  Author

	(Author+name "Viencino Campi")
	(Nationality [Ontologia_Class50003]))

([Ontologia_Class60044] of  Painting

	(Created+by [Ontologia_Class60045])
	(Exhibited+in [Ontologia_Class10002])
	(Height 800)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "Concert")
	(Painting+style [Ontologia_Class50002])
	(Painting+topic [Ontologia_Class60022])
	(Relevance 10)
	(Visitor+interest -1)
	(Width 506)
	(Year+of+creation 1590))

([Ontologia_Class60045] of  Author

	(Author+name "Leandro Bassano")
	(Nationality [Ontologia_Class50003]))

([Ontologia_Class60046] of  Painting

	(Created+by [Ontologia_Class60047])
	(Exhibited+in [Ontologia_Class30000])
	(Height 280)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "Creacion de Adan")
	(Painting+style [Ontologia_Class60048])
	(Painting+topic [Ontologia_Class30009])
	(Relevance 60)
	(Visitor+interest -1)
	(Width 570)
	(Year+of+creation 1510))

([Ontologia_Class60047] of  Author

	(Author+name "Miguel Angel")
	(Nationality [Ontologia_Class50003]))

([Ontologia_Class60048] of  Style

	(Style+name "Cinquecento"))

([Ontologia_Class60049] of  Painting

	(Created+by [Ontologia_Class60050])
	(Exhibited+in [Ontologia_Class40003])
	(Height 104)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "Dama y Ninya")
	(Painting+style [Ontologia_Class50002])
	(Painting+topic [Ontologia_Class10008])
	(Relevance 0)
	(Visitor+interest -1)
	(Width 78)
	(Year+of+creation 1587))

([Ontologia_Class60050] of  Author

	(Author+name "Adrien Cronenburch")
	(Nationality [Ontologia_Class50004]))

([Ontologia_Class60051] of  Painting

	(Created+by [Ontologia_Class30012])
	(Exhibited+in [Ontologia_Class40004])
	(Height 800)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Desnudo femenino echado")
	(Painting+style [Ontologia_Class40019])
	(Painting+topic [Ontologia_Class10008])
	(Relevance 30)
	(Visitor+interest -1)
	(Width 470)
	(Year+of+creation 1887))

([Ontologia_Class60052] of  Painting

	(Created+by [Ontologia_Class60036])
	(Exhibited+in [Ontologia_Class60053])
	(Height 80)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "El estanque de los nenufares")
	(Painting+style [Ontologia_Class40019])
	(Painting+topic [Ontologia_Class20002])
	(Relevance 50)
	(Visitor+interest -1)
	(Width 78)
	(Year+of+creation 1899))

([Ontologia_Class60053] of  Room

	(Number 5))

([Ontologia_Class60054] of  Painting

	(Created+by [Ontologia_Class60056])
	(Exhibited+in [Ontologia_Class60053])
	(Height 82)
	(Observation+time -1)
	(Painted+in [Ontologia_Class10000])
	(Painting+name "El matrimonio Arnolfini")
	(Painting+style [Ontologia_Class60055])
	(Painting+topic [Ontologia_Class60022])
	(Relevance 30)
	(Visitor+interest -1)
	(Width 60)
	(Year+of+creation 1434))

([Ontologia_Class60055] of  Style

	(Style+name "Gotico flamenco"))

([Ontologia_Class60056] of  Author

	(Author+name "Jan Van Eyck")
	(Nationality [Ontologia_Class60057]))

([Ontologia_Class60057] of  Country

	(Country+name "Flandes"))

([Ontologia_Class60058] of  Painting

	(Created+by [Ontologia_Class60059])
	(Exhibited+in [Ontologia_Class60053])
	(Height 33)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "El sacrificio a Pan")
	(Painting+style [Ontologia_Class20003])
	(Painting+topic [Ontologia_Class60013])
	(Relevance 10)
	(Visitor+interest -1)
	(Width 24)
	(Year+of+creation 1771))

([Ontologia_Class60059] of  Author

	(Author+name "Francisco de Goya")
	(Nationality [Ontologia_Class50009]))

([Ontologia_Class60060] of  Painting

	(Created+by [Ontologia_Class60028])
	(Exhibited+in [Ontologia_Class60053])
	(Height 349)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Gernika")
	(Painting+style [Ontologia_Class30004])
	(Painting+topic [Ontologia_Class30010])
	(Relevance 80)
	(Visitor+interest -1)
	(Width 777)
	(Year+of+creation 1937))

([Ontologia_Class60061] of  Painting

	(Created+by [Ontologia_Class60056])
	(Exhibited+in [Ontologia_Class60062])
	(Height 56)
	(Observation+time -1)
	(Painted+in [Ontologia_Class10000])
	(Painting+name "La crucifixion")
	(Painting+style [Ontologia_Class60055])
	(Painting+topic [Ontologia_Class30009])
	(Relevance 40)
	(Visitor+interest -1)
	(Width 19)
	(Year+of+creation 1425))

([Ontologia_Class60062] of  Room

	(Number 6))

([Ontologia_Class60063] of  Painting

	(Created+by [Ontologia_Class60036])
	(Exhibited+in [Ontologia_Class60062])
	(Height 61)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "La escarcha")
	(Painting+style [Ontologia_Class40019])
	(Painting+topic [Ontologia_Class20002])
	(Relevance 10)
	(Visitor+interest -1)
	(Width 81)
	(Year+of+creation 1880))

([Ontologia_Class60064] of  Painting

	(Created+by [Ontologia_Class60065])
	(Exhibited+in [Ontologia_Class60062])
	(Height 500)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "La escuela de Atenas")
	(Painting+style [Ontologia_Class60048])
	(Painting+topic [Ontologia_Class60022])
	(Relevance 20)
	(Visitor+interest -1)
	(Width 770)
	(Year+of+creation 1509))

([Ontologia_Class60065] of  Author

	(Author+name "Rafael de Urbino")
	(Nationality [Ontologia_Class50003]))

([Ontologia_Class60066] of  Painting

	(Created+by [Ontologia_Class60032])
	(Exhibited+in [Ontologia_Class60062])
	(Height 223)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "La fragua de Vulcano")
	(Painting+style [Ontologia_Class60038])
	(Painting+topic [Ontologia_Class60013])
	(Relevance 40)
	(Visitor+interest -1)
	(Width 290)
	(Year+of+creation 1630))

([Ontologia_Class60067] of  Painting

	(Created+by [Ontologia_Class30012])
	(Exhibited+in [Ontologia_Class60053])
	(Height 72)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "La habitación de Vincent en Arlés")
	(Painting+style [Ontologia_Class30003])
	(Painting+topic [Ontologia_Class60022])
	(Relevance 40)
	(Visitor+interest -1)
	(Width 90)
	(Year+of+creation 1888))

([Ontologia_Class60068] of  Painting

	(Created+by [Ontologia_Class60059])
	(Exhibited+in [Ontologia_Class60053])
	(Height 98)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "La maja desnuda")
	(Painting+style [Ontologia_Class30007])
	(Painting+topic [Ontologia_Class10008])
	(Relevance 70)
	(Visitor+interest -1)
	(Width 191)
	(Year+of+creation 1790))

([Ontologia_Class60069] of  Painting

	(Created+by [Ontologia_Class60032])
	(Exhibited+in [Ontologia_Class60053])
	(Height 220)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "Las Hilanderas")
	(Painting+style [Ontologia_Class60038])
	(Painting+topic [Ontologia_Class60013])
	(Relevance 40)
	(Visitor+interest -1)
	(Width 289)
	(Year+of+creation 1657))

([Ontologia_Class60070] of  Painting

	(Created+by [Ontologia_Class60032])
	(Exhibited+in [Ontologia_Class60053])
	(Height 318)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "Las meninas")
	(Painting+style [Ontologia_Class60038])
	(Painting+topic [Ontologia_Class10008])
	(Relevance 80)
	(Visitor+interest -1)
	(Width 276)
	(Year+of+creation 1656))

([Ontologia_Class60071] of  Painting

	(Created+by [Ontologia_Class60028])
	(Exhibited+in [Ontologia_Class60062])
	(Height 243)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Las señoritas de Avignon")
	(Painting+style [Ontologia_Class60072])
	(Painting+topic [Ontologia_Class60022])
	(Relevance 50)
	(Visitor+interest -1)
	(Width 234)
	(Year+of+creation 1907))

([Ontologia_Class60072] of  Style

	(Style+name "Cubismo"))

([Ontologia_Class60073] of  Painting

	(Created+by [Ontologia_Class60074])
	(Exhibited+in [Ontologia_Class60062])
	(Height 221)
	(Observation+time -1)
	(Painted+in [Ontologia_Class20004])
	(Painting+name "Las tres gracias")
	(Painting+style [Ontologia_Class60075])
	(Painting+topic [Ontologia_Class60013])
	(Relevance 40)
	(Visitor+interest -1)
	(Width 181)
	(Year+of+creation 1636))

([Ontologia_Class60074] of  Author

	(Author+name "Pedro Pablo Rubens")
	(Nationality [Ontologia_Class50004]))

([Ontologia_Class60075] of  Style

	(Style+name "Escuela flamenca"))

([Ontologia_Class60076] of  Painting

	(Created+by [Ontologia_Class60059])
	(Exhibited+in [Ontologia_Class60062])
	(Height 226)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Los fusilamientos del tres de mayo")
	(Painting+style [Ontologia_Class20003])
	(Painting+topic [Ontologia_Class30010])
	(Relevance 60)
	(Visitor+interest -1)
	(Width 345)
	(Year+of+creation 1814))

([Ontologia_Class60078] of  Painting

	(Created+by [Ontologia_Class60020])
	(Exhibited+in [Ontologia_Class60062])
	(Height 88)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Maria Rusiñol en el Cau Ferrat")
	(Painting+style [Ontologia_Class60080])
	(Painting+topic [Ontologia_Class60022])
	(Relevance 5)
	(Visitor+interest -1)
	(Width 79)
	(Year+of+creation 1894))

([Ontologia_Class60080] of  Style

	(Style+name "Modernismo"))

([Ontologia_Class60081] of  Painting

	(Created+by [Ontologia_Class60036])
	(Exhibited+in [Ontologia_Class60053])
	(Height 800)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Mujer en el jardín de Saint-Adresse")
	(Painting+style [Ontologia_Class40019])
	(Painting+topic [Ontologia_Class60022])
	(Relevance 10)
	(Visitor+interest -1)
	(Width 684)
	(Year+of+creation 1867))

([Ontologia_Class60082] of  Painting

	(Created+by [Ontologia_Class30027])
	(Exhibited+in [Ontologia_Class60053])
	(Height 260)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Napoleón cruzando los Alpes")
	(Painting+style [Ontologia_Class30007])
	(Painting+topic [Ontologia_Class10008])
	(Relevance 10)
	(Visitor+interest -1)
	(Width 221)
	(Year+of+creation 1801))

([Ontologia_Class60083] of  Painting

	(Created+by [Ontologia_Class60085])
	(Exhibited+in [Ontologia_Class60053])
	(Height 30)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Naturaleza muerta")
	(Painting+style [Ontologia_Class40007])
	(Painting+topic [Ontologia_Class20002])
	(Relevance 40)
	(Visitor+interest -1)
	(Width 41)
	(Year+of+creation 1866))

([Ontologia_Class60085] of  Author

	(Author+name "Paul Cezanne")
	(Nationality [Ontologia_Class50010]))

([Ontologia_Class60086] of  Painting

	(Created+by [Ontologia_Class60043])
	(Exhibited+in [Ontologia_Class60053])
	(Height 32)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Paseo con dos figuras")
	(Painting+style [Ontologia_Class40007])
	(Painting+topic [Ontologia_Class20002])
	(Relevance 10)
	(Visitor+interest -1)
	(Width 40)
	(Year+of+creation 1885))

([Ontologia_Class60087] of  Painting

	(Created+by [Ontologia_Class60059])
	(Exhibited+in [Ontologia_Class60062])
	(Height 146)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Saturno devorando a un hijo")
	(Painting+style [Ontologia_Class30007])
	(Painting+topic [Ontologia_Class60013])
	(Relevance 60)
	(Visitor+interest -1)
	(Width 83)
	(Year+of+creation 1821))

([Ontologia_Class60088] of  Painting

	(Created+by [Ontologia_Class60090])
	(Exhibited+in [Ontologia_Class60062])
	(Height 208)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Tarde de domingo en la isla de la Grande Jatte")
	(Painting+style [Ontologia_Class60089])
	(Painting+topic [Ontologia_Class20002])
	(Relevance 40)
	(Visitor+interest -1)
	(Width 308)
	(Year+of+creation 1884))

([Ontologia_Class60089] of  Style

	(Style+name "Puntilismo"))

([Ontologia_Class60090] of  Author

	(Author+name "Georges Seurat")
	(Nationality [Ontologia_Class50010]))

([Ontologia_Class60091] of  Painting

	(Created+by [Ontologia_Class60028])
	(Exhibited+in [Ontologia_Class60062])
	(Height 61)
	(Observation+time -1)
	(Painted+in [Ontologia_Class30002])
	(Painting+name "Una cantera")
	(Painting+style [Ontologia_Class40007])
	(Painting+topic [Ontologia_Class20002])
	(Relevance 10)
	(Visitor+interest -1)
	(Width 82)
	(Year+of+creation 1896))


)